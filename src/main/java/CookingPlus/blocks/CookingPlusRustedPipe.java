...
public class CookingPlusRustedPipe extends CookingPlusCustomBlock{

	private final String name = "rustedpipe";
	...
	public CookingPlusRustedPipe() {
		super(Material.IRON);
		GameRegistry.registerBlock(this, name);
		this.setUnlocalizedName("rustedpipe");
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos)
    {
	 ...
	 public boolean isValidAttachmentBlock(Block myBlock, IBlockAccess myWorld, BlockPos myPos){
		 if(myWorld.getBlockState(myPos).getBlock() == CookingPlusMain.blockRustedPipe){
			 return true;
		 }else if(myWorld.getBlockState(myPos).getBlock() == CookingPlusMain.blockRustedBot){
			 return true;
		 }
		 else{
			 return myWorld.getBlockState(myPos).isOpaqueCube() && myWorld.getBlockState(myPos).isFullCube();
		 }
	 }
