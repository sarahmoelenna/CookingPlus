...
public class CookingPlusSaucePanBlock extends CookingPlusCustomTileEntityBlock {
	    ...
	    public void onBlockPlacedBy(World worldIn, BlockPos myPos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	    {
	        ...
	        if (tileentity instanceof SaucepanTileEntity)
            {
	        	if(enumfacing == EnumFacing.EAST){
	        		((SaucepanTileEntity)tileentity).setDirection(5);
	        	}
	        	if(enumfacing == EnumFacing.SOUTH){
	        		((SaucepanTileEntity)tileentity).setDirection(3);
	        	}
	        	if(enumfacing == EnumFacing.WEST){
	        		((SaucepanTileEntity)tileentity).setDirection(4);
	        	}
	        	if(enumfacing == EnumFacing.NORTH){
	        		((SaucepanTileEntity)tileentity).setDirection(2);
	        	}
            }
	    }
		...
		public List<ItemStack> getDrops(IBlockAccess world, BlockPos myPos, IBlockState myState, int fortune)
	    {
	        ...
	        ret.add(new ItemStack(Item.getItemFromBlock(CookingPlusMain.blockSaucePan), 1, 0));
	        if(world.getTileEntity(myPos) != null){
	        	ItemStack[] TempArray = ((SaucepanTileEntity)world.getTileEntity(myPos)).GetItems();
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	            ...
	            if (tileentity instanceof SaucepanTileEntity)
	            {
	                InventoryHelper.dropInventoryItems(worldIn, pos, (SaucepanTileEntity)tileentity);
	            }
