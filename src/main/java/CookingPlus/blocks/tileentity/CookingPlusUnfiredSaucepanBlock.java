...
public class CookingPlusUnfiredSaucepanBlock extends CookingPlusCustomTileEntityBlock {

        //Treat it like a normal block here. The Block Bounds are a good idea - the first three are X Y and Z of the botton-left corner,
        //And the second three are the top-right corner.
		private final String name = "unfiredsaucepan";
		public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
        public CookingPlusUnfiredSaucepanBlock() {
                super(Material.clay);
		...
		public TileEntity createNewTileEntity(World var1, int var2) {
			return new UnfiredSaucepanTileEntity();
		}
	    ...
	    public void onBlockPlacedBy(World worldIn, BlockPos myPos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	    {
	        ...
	        if (tileentity instanceof UnfiredSaucepanTileEntity)
            {
	        	if(enumfacing == EnumFacing.EAST){
	        		((UnfiredSaucepanTileEntity)tileentity).setDirection(5);
	        	}
	        	if(enumfacing == EnumFacing.SOUTH){
	        		((UnfiredSaucepanTileEntity)tileentity).setDirection(3);
	        	}
	        	if(enumfacing == EnumFacing.WEST){
	        		((UnfiredSaucepanTileEntity)tileentity).setDirection(4);
	        	}
	        	if(enumfacing == EnumFacing.NORTH){
	        		((UnfiredSaucepanTileEntity)tileentity).setDirection(2);
	        	}
            }
	    }
		...
		public List<ItemStack> getDrops(IBlockAccess world, BlockPos myPos, IBlockState myState, int fortune)
	    {
	        ...
	        ret.add(new ItemStack(Item.getItemFromBlock(CookingPlusMain.blockUnfiredSaucePan), 1, 0));
	        if(world.getTileEntity(myPos) != null){
	        	ItemStack[] TempArray = ((UnfiredSaucepanTileEntity)world.getTileEntity(myPos)).GetItems();
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	            ...
	            if (tileentity instanceof UnfiredSaucepanTileEntity)
	            {
	                InventoryHelper.dropInventoryItems(worldIn, pos, (UnfiredSaucepanTileEntity)tileentity);
	            }
