...
public class CookingPlusSheetPressBlock extends CookingPlusCustomTileEntityBlock {
		...
		public TileEntity createNewTileEntity(World var1, int var2) {
			return new SheetPressTileEntity();
		}
	    ...
	    public void onBlockPlacedBy(World worldIn, BlockPos myPos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	    {
	        ...
	        if (tileentity instanceof SheetPressTileEntity)
            {
	        	if(enumfacing == EnumFacing.EAST){
	        		((SheetPressTileEntity)tileentity).setDirection(5);
	        	}
	        	if(enumfacing == EnumFacing.SOUTH){
	        		((SheetPressTileEntity)tileentity).setDirection(3);
	        	}
	        	if(enumfacing == EnumFacing.WEST){
	        		((SheetPressTileEntity)tileentity).setDirection(4);
	        	}
	        	if(enumfacing == EnumFacing.NORTH){
	        		((SheetPressTileEntity)tileentity).setDirection(2);
	        	}
            }
	    }
		...
		public List<ItemStack> getDrops(IBlockAccess world, BlockPos myPos, IBlockState myState, int fortune)
	    {
	        ...
	        ret.add(new ItemStack(Item.getItemFromBlock(CookingPlusMain.blockSheetPress), 1, 0));
	        if(world.getTileEntity(myPos) != null){
	        	ItemStack[] TempArray = ((SheetPressTileEntity)world.getTileEntity(myPos)).GetItems();
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	            ...
	            if (tileentity instanceof SheetPressTileEntity)
	            {
	                InventoryHelper.dropInventoryItems(worldIn, pos, (SheetPressTileEntity)tileentity);
	            }
