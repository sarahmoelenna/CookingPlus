...
public class CookingPlusMarketBoxBlock extends CookingPlusCustomTileEntityBlock {
		...
		public TileEntity createNewTileEntity(World var1, int var2) {
			return new MarketBoxTileEntity();
		}
	    ...
	    public void onBlockPlacedBy(World worldIn, BlockPos myPos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	    {
	        ...
	        if (tileentity instanceof MarketBoxTileEntity)
            {
	        	if(enumfacing == EnumFacing.EAST){
	        		((MarketBoxTileEntity)tileentity).setDirection(5);
	        	}
	        	if(enumfacing == EnumFacing.SOUTH){
	        		((MarketBoxTileEntity)tileentity).setDirection(3);
	        	}
	        	if(enumfacing == EnumFacing.WEST){
	        		((MarketBoxTileEntity)tileentity).setDirection(4);
	        	}
	        	if(enumfacing == EnumFacing.NORTH){
	        		((MarketBoxTileEntity)tileentity).setDirection(2);
	        	}
            }
	    }
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ)
	    {
					...
					if(worldIn.getTileEntity(pos) != null){
						if(worldIn.getTileEntity(pos) instanceof MarketBoxTileEntity){
							MarketBoxTileEntity MyMarketBox = (MarketBoxTileEntity) worldIn.getTileEntity(pos);
		...
		public List<ItemStack> getDrops(IBlockAccess world, BlockPos myPos, IBlockState myState, int fortune)
	    {
	        ...
	        ret.add(new ItemStack(Item.getItemFromBlock(CookingPlusMain.blockMarketBox), 1, 0));
	        //System.out.println("blah");
	        if(world.getTileEntity(myPos) != null){
	        	//System.out.println("hmm");
	        	ItemStack[] TempArray = ((MarketBoxTileEntity)world.getTileEntity(myPos)).GetItems();
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	            ...
	            if (tileentity instanceof MarketBoxTileEntity)
	            {
	                InventoryHelper.dropInventoryItems(worldIn, pos, (MarketBoxTileEntity)tileentity);
	            }
