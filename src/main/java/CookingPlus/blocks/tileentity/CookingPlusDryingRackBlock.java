...
public class CookingPlusDryingRackBlock extends CookingPlusCustomTileEntityBlock {
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ)
	    {
					...
					if(worldIn.getTileEntity(pos) != null){
						if(worldIn.getTileEntity(pos) instanceof DryingRackTileEntity){
							DryingRackTileEntity MyMarketBox = (DryingRackTileEntity) worldIn.getTileEntity(pos);
		...
		public List<ItemStack> getDrops(IBlockAccess world, BlockPos myPos, IBlockState myState, int fortune)
	    {
	        ...
	        ret.add(new ItemStack(Item.getItemFromBlock(CookingPlusMain.blockDryingRack), 1, 0));
	        //System.out.println("blah");
	        if(world.getTileEntity(myPos) != null){
	        	//System.out.println("hmm");
	        	ItemStack[] TempArray = ((DryingRackTileEntity)world.getTileEntity(myPos)).GetItems();
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	            ...
	            if (tileentity instanceof DryingRackTileEntity)
	            {
	                InventoryHelper.dropInventoryItems(worldIn, pos, (DryingRackTileEntity)tileentity);
	            }
	    ...
	    {
	    	int meta = ((EnumFacing)worldIn.getBlockState(pos).getValue(FACING)).getIndex();
