...
public class CookingPlusVatBlock extends CookingPlusCustomTileEntityBlock {
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
	    {
        	if(!worldIn.isRemote)
            {
                    VatTileEntity t = (VatTileEntity) worldIn.getTileEntity(pos);
	    ...
	    public void onBlockPlacedBy(World worldIn, BlockPos myPos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	    {
	        ...
	        if (tileentity instanceof VatTileEntity)
            {
	        	if(enumfacing == EnumFacing.EAST){
	        		((VatTileEntity)tileentity).setDirection(5);
	        	}
	        	if(enumfacing == EnumFacing.SOUTH){
	        		((VatTileEntity)tileentity).setDirection(3);
	        	}
	        	if(enumfacing == EnumFacing.WEST){
	        		((VatTileEntity)tileentity).setDirection(4);
	        	}
	        	if(enumfacing == EnumFacing.NORTH){
	        		((VatTileEntity)tileentity).setDirection(2);
	        	}
            }
	    }
		...
		public List<ItemStack> getDrops(IBlockAccess world, BlockPos myPos, IBlockState myState, int fortune)
	    {
	        ...
	        ret.add(new ItemStack(Item.getItemFromBlock(CookingPlusMain.blockVat), 1, 0));
	        if(world.getTileEntity(myPos) != null){
	        	ItemStack[] TempArray = ((VatTileEntity)world.getTileEntity(myPos)).GetItems();
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	            ...
	            if (tileentity instanceof VatTileEntity)
	            {
	                InventoryHelper.dropInventoryItems(worldIn, pos, (VatTileEntity)tileentity);
	            }
