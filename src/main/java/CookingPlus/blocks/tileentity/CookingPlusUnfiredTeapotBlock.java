...
public class CookingPlusUnfiredTeapotBlock extends CookingPlusCustomTileEntityBlock {
		...
		public TileEntity createNewTileEntity(World var1, int var2) {
			return new UnfiredTeapotTileEntity();
		}
	    ...
	    public void onBlockPlacedBy(World worldIn, BlockPos myPos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	    {
	        ...
	        if (tileentity instanceof UnfiredTeapotTileEntity)
            {
	        	if(enumfacing == EnumFacing.EAST){
	        		((UnfiredTeapotTileEntity)tileentity).setDirection(5);
	        	}
	        	if(enumfacing == EnumFacing.SOUTH){
	        		((UnfiredTeapotTileEntity)tileentity).setDirection(3);
	        	}
	        	if(enumfacing == EnumFacing.WEST){
	        		((UnfiredTeapotTileEntity)tileentity).setDirection(4);
	        	}
	        	if(enumfacing == EnumFacing.NORTH){
	        		((UnfiredTeapotTileEntity)tileentity).setDirection(2);
	        	}
            }
	    }
		...
		public List<ItemStack> getDrops(IBlockAccess world, BlockPos myPos, IBlockState myState, int fortune)
	    {
	        ...
	        ret.add(new ItemStack(Item.getItemFromBlock(CookingPlusMain.blockUnfiredTeapot), 1, 0));
	        if(world.getTileEntity(myPos) != null){
	        	ItemStack[] TempArray = ((UnfiredTeapotTileEntity)world.getTileEntity(myPos)).GetItems();
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	            ...
	            if (tileentity instanceof UnfiredTeapotTileEntity)
	            {
	                InventoryHelper.dropInventoryItems(worldIn, pos, (UnfiredTeapotTileEntity)tileentity);
	            }
