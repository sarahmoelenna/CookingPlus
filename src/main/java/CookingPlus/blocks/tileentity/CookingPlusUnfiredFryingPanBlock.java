...
public class CookingPlusUnfiredFryingPanBlock extends BlockContainer {
        ...
        public CookingPlusUnfiredFryingPanBlock() {
                super(Material.clay);
                this.setUnlocalizedName("unfiredfryingpan");
	    ...
	    public void onBlockPlacedBy(World worldIn, BlockPos myPos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	    {
	        ...
	        if (tileentity instanceof UnfiredFryingPanTileEntity)
            {
	        	if(enumfacing == EnumFacing.EAST){
	        		((UnfiredFryingPanTileEntity)tileentity).setDirection(5);
	        	}
	        	if(enumfacing == EnumFacing.SOUTH){
	        		((UnfiredFryingPanTileEntity)tileentity).setDirection(3);
	        	}
	        	if(enumfacing == EnumFacing.WEST){
	        		((UnfiredFryingPanTileEntity)tileentity).setDirection(4);
	        	}
	        	if(enumfacing == EnumFacing.NORTH){
	        		((UnfiredFryingPanTileEntity)tileentity).setDirection(2);
	        	}
            }
	    }
		...
		public List<ItemStack> getDrops(IBlockAccess world, BlockPos myPos, IBlockState myState, int fortune)
	    {
	        ...
	        ret.add(new ItemStack(Item.getItemFromBlock(CookingPlusMain.blockUnfiredFryingPan), 1, 0));
	        if(world.getTileEntity(myPos) != null){
	        	ItemStack[] TempArray = ((UnfiredFryingPanTileEntity)world.getTileEntity(myPos)).GetItems();
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	            ...
	            if (tileentity instanceof UnfiredFryingPanTileEntity)
	            {
	                InventoryHelper.dropInventoryItems(worldIn, pos, (UnfiredFryingPanTileEntity)tileentity);
	            }
