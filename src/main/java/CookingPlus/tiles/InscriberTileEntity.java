...
public class InscriberTileEntity extends CookingPlusCustomTileEntity implements ITickable, IInventory {
	...
	protected boolean isItemDone;
	protected boolean resetMovement;
	
	public InscriberTileEntity(){
		inv = new ItemStack[1];
    ...
    public void update(){
		...
		if(usedByBot == true){
			if(shouldProcess() == true || resetMovement == true){
				moveModel();
			}
		}
	}
	...
	public void moveModel(){
		if(rodState == true && resetMovement == false){
			if(rodMovement <= 0.62){
		...
		if(toolState == true && resetMovement == false){
			if(toolMovement <= 0.6){
	...
	public void processSlots(){
		if(isItemDone == true){
			setInventorySlotContents(0, InscriptionRecipeHelper.instance().getInscriptionResult(inv[0]));
			isItemDone = false;
	...
	public boolean shouldProcess(){
		return InscriptionRecipeHelper.instance().isItemValidInput(inv[0]);
	}
    ...
    {
		if(oldState.getBlock().equals(CookingPlusMain.blockInscriber) == true){
			if(newSate.getBlock().equals(CookingPlusMain.blockInscriber) == false){
