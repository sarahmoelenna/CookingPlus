...
public class DryingRackTileEntity extends CookingPlusCustomTileEntity implements IInventory, ITickable {
    ...
    {
		if(oldState.getBlock().equals(CookingPlusMain.blockDryingRack) == true){
			if(newSate.getBlock().equals(CookingPlusMain.blockDryingRack) == false){
	...
	public void ProcessSlot(){
		if(inv[0] != null){
			if(CookingPlusDryingRackRecipe.instance().getDryingRackResult(inv[0]) != null){
				if(timer > 4000){
					inv[0] = new ItemStack(CookingPlusDryingRackRecipe.instance().getDryingRackResult(inv[0]).getItem(), 1);
					timer = 0;
				}
			}
			else if(inv[0].getItem() == CookingPlusMain.soakedbook){
				if(timer > 4000){
					inv[0] = getRandomPotteryGuide();
					timer = 0;
				}
			}
			else{
	...
	public ItemStack getRandomPotteryGuide(){
		Random myRand = new Random();
		return new ItemStack(CookingPlusLootHelper.instance().GetRandomGuide(myRand));
	}
}
